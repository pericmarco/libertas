...
// Produktneutrales Kartendialog-Modul: Bürger:innen setzen einen Punkt auf die
// Karte, wählen eine Art und schreiben einen Hinweis; andere unterstützen.
// Alle Inhalte kommen per Props — nutzbar in Musterstadt UND Network.
export type DialogCategory = { key: string; label: string; emoji: string; color: string }
export type DialogContribution = { id: string; lng: number; lat: number; category: string; text: string; author: string; supports: number }

export default function Kartendialog({
  center,
  zoom = 15,
  prompt = 'Setzen Sie einen Punkt auf die Karte und sagen Sie uns, worum es geht.',
  title = 'Kartendialog',
  categories,
  initial,
}: {
  ...
  initial: DialogContribution[]
}) {
  const catMap = useMemo(() => Object.fromEntries(categories.map(c => [c.key, c])) as Record<string, DialogCategory>, [categories])
  const [items, setItems] = useState<DialogContribution[]>(initial)
  ...
  const pins: MapPinT[] = useMemo(() => items.map(c => ({
    id: c.id, lng: c.lng, lat: c.lat,
    title: catMap[c.category]?.label ?? '',
    meta: c.text,
    color: catMap[c.category]?.color ?? '#2563EB',
  })), [items, catMap])

  function addContribution() {
    if (!draft || text.trim().length < 3) return
    setItems(prev => [{ id: 'neu-' + nextId.current++, lng: draft.lng, lat: draft.lat, category: cat, text: text.trim(), author: 'Sie (Demo)', supports: 0 }, ...prev])
    setDraft(null); setText('')
  }
  function toggleSupport(id: string) {
    setSupported(prev => { const n = new Set(prev); if (n.has(id)) n.delete(id); else n.add(id); return n })
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <MapPanel
        picker maxPins={1} geolocate title={title}
        center={center} zoom={zoom} pins={pins}
        value={draft ? [draft] : []} onChange={p => setDraft(p[0] ?? null)} onPinClick={setActive}
            ...
            {categories.map(c => {
              ...
              return (
                <button key={c.key} onClick={() => setCat(c.key)}
                  ...
                  style={on ? { backgroundColor: c.color, borderColor: 'transparent' } : undefined}>
                  <span>{c.emoji}</span> {c.label}
                </button>
              )
            })}
          </div>
          <textarea value={text} onChange={e => setText(e.target.value)} rows={3} placeholder="Beschreiben Sie kurz Ihren Hinweis…"
        ...
        {sorted.map(c => {
          ...
          return (
            <div key={c.id}
              ...
              <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-sm" style={{ backgroundColor: color + '1a', color }}>
                {m?.emoji}
              </span>
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="rounded-full px-2 py-0.5 text-[10px] font-semibold" style={{ backgroundColor: color + '1a', color }}>{m?.label}</span>
                  <span className="text-xs text-gray-400">{c.author}</span>
                </div>
                <p className="mt-1 text-sm leading-relaxed text-gray-700">{c.text}</p>
              </div>
              <button onClick={() => toggleSupport(c.id)}
